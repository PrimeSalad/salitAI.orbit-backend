...
const GEMINI_MODEL_MINUTES = String(process.env.GEMINI_MODEL_MINUTES ?? "").trim();
...
const MAX_UPLOAD_MB = Number(process.env.MAX_UPLOAD_MB ?? DEFAULT_UPLOAD_LIMIT_MB);
...
async function ensure_temp_directory() {
    await fsp.mkdir(TEMP_UPLOAD_DIR, { recursive: true });
}
function build_temp_filename(original_name) {
...
function supports_generate_content(model) {
...
async function list_models(api_key) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`ListModels failed (${response.status}): ${error_text || response.statusText}`);
    }
    const json = (await response.json().catch(() => ({})));
    return Array.isArray(json.models) ? json.models : [];
}
function build_candidate_model_pool(available_models, preferred_model) {
    const supported_models = available_models
        .filter((model) => supports_generate_content(model) && model.name)
        .map((model) => normalize_model_name(model.name));
...
async function get_model_pool(preferred_model) {
    const now = Date.now();
    if (cached_model_names.length > 0 &&
    ...
    if (!pool.length) {
        throw new Error("No available Gemini model supports generateContent for this API key.");
    }
    if (!preferred_model) {
...
function is_retryable_model_error(error) {
...
async function run_with_model_fallback(preferred_model, operation) {
    ...
    for (const model_name of model_pool) {
        try {
            ...
            const error_message = String(error instanceof Error ? error.message : error ?? "Unknown error");
            failures.push(`${model_name}: ${error_message}`);
            if (!is_retryable_model_error(error)) {
                throw new Error(`Model ${model_name} failed with non-retryable error: ${error_message}`);
            }
        }
    }
    throw new Error(`All candidate models failed. Attempts: ${failures.join(" | ")}`);
}
/* ============================= */
/* FILE HELPERS                  */
/* ============================= */
async function safe_unlink(file_path) {
    if (!file_path) {
        return;
    }
    try {
...
async function safe_delete_uploaded_file(file_name) {
    if (!file_name) {
        return;
    }
    try {
...
function resolve_audio_mime_type(file) {
...
async function upload_audio_file_to_gemini(file) {
...
/* CONTACT EMAIL                 */
/* ============================= */
const mailer = SMTP_USER && SMTP_PASS && CONTACT_TO_EMAIL
...
app.post("/api/stt", upload.single("audio"), async (req, res) => {
    ...
    try {
        ...
        const transcription_prompt = [
            createPartFromUri(String(uploaded_gemini_file.uri), String(uploaded_gemini_file.mimeType)),
            [
        ...
        const result = await run_with_model_fallback(GEMINI_MODEL_STT || undefined, async (model_name) => {
...
app.post("/api/minutes", async (req, res) => {
    try {
        ...
        const document_type = String(req.body?.document_type ?? "Executive Meeting Minutes").trim();
...
${transcript}
    `.trim();
        const result = await run_with_model_fallback(GEMINI_MODEL_MINUTES || undefined, async (model_name) => {
            const response = await ai.models.generateContent({
                model: model_name,
                contents: prompt,
            });
