...
export async function createApp() {
    ...
    const sttController = new STTController(audioService, geminiService, config.gemini.modelSTT);
    const minutesController = new MinutesController(geminiService, config.gemini.modelMinutes);
    ...
    // Upload middleware
    const upload = createUploadMiddleware(config.upload.tempDir, config.upload.maxSizeMB);
    // Routes
    const routes = createRoutes(upload, sttController, minutesController, contactController, geminiService);
    app.use(routes);
    // Error handling
